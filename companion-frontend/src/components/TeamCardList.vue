<template>
  <div id="teamCardList">
    ...
    >
      ...
      <template #bottom>
        <div>
          {{ `Current People: ${team.hasJoinNum}/${team.maxNum}`}}
        </div>
      ...
      <template #footer>
        <van-button size="mini" color="#1989FA" v-if="team.userId !== currentUser?.id  && !team.hasJoin"
          ...
          Join</van-button>
        <van-button v-if="team.userId === currentUser?.id" size="mini" color="#1989FA" @click.stop="doUpdateTeam(team.id)">Update</van-button>
        <van-button size="mini" v-if="team.hasJoin" color="#1989FA" @click.stop="doQuitTeam(team.id)">Exit</van-button>
        <van-button v-if="team.userId === currentUser?.id" size="mini" type="danger" plain @click.stop="doDeleteTeam(team.id)">Delete</van-button>
      </template>
    </van-card>
    <!--输入密码框-->
    <van-dialog v-model:show="showPasswordDialog" title="Password" show-cancel-button @confirm="doJoinTeam" @cancel="cancelJoin">
  ...
  const doJoinTeam = async () => {
    ...
    if (res?.code === 0) {
      ...
      showFailToast("Failure" + (res.description ? `， ${res.description} `:''));
    }
  ...
  const doQuitTeam = async(id: number) => {
    ...
    if (res?.code === 0) {
      showSuccessToast("Exit Success");
    } else {
      showFailToast("Exit Failure");
    }
  }
