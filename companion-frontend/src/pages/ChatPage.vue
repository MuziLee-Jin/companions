    ...
    <van-cell-group inset style="position: fixed;bottom: 0;width: 100%">
      ...
      >
        <template #button style="margin-bottom: 100px">
          <van-button size="small" type="primary" @click="send" style="margin-right: 16px">Send</van-button>
        </template>
      </van-field>
    </van-cell-group>
...
const init = () => {
  ...
  if (typeof (WebSocket) == "undefined") {
    showFailToast("Your browser does not support  WebSocket")
  } else {
...
const send = () => {
  ...
  if (stats.value.chatUser.id === stats.value.user.id) {
    showFailToast("You can't text yourself")
    return;
  }
  if (!stats.value.text.trim()) {
    showFailToast("Please enter content")
  } else {
    if (typeof (WebSocket) == "undefined") {
      showFailToast("Your browser does not support WebSocket")
    } else {
